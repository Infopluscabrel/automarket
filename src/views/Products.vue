...
export default {
  ...
  methods: {
    ...
    // filterProductBy(event, productFilterID,name) {
    filterProductBy(event,name) {
      const productList = [...this.products_list];
      let filteredData = [];
      if (event.target.value !== "All") {
        productList.map(product => {
          ...
          if(product.productName){
            ...
            if (   product.productName.includes(name)  && !product.isSold) {
            filteredData.push(product);
    ...
    filterProductByPrice(event,name) {
      const productList = [...this.products_list];
      let filteredData = [];
      if (event.target.value !== "All") {
        productList.map(product => {
          ...
          if(product.productName){
            ...
            console.log(parseFloat(product.productPrice))
            console.log(parseFloat(product.productPrice)<=name)
            if (   parseFloat(product.productPrice)<=name  && !product.isSold) {
            filteredData.push(product);
